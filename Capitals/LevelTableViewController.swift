...
//  Capitals
//
//  Created by Samvid Konchada on 1/4/20.
...
self.navigationItem.setHidesBackButton( true, animated: false)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        audioPlayer.play()
           performSegue(withIdentifier: "cellClicked", sender: selectedRegion)
       }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let DesVC = segue.destination as! TestViewController
        let selectedRegion = sender as! Region
        DesVC.myRegion = selectedRegion
    }
    
    @IBAction func home4(_ sender: Any) {
        audioPlayer.play()
